...
//                       <p>Chart debug area</p>
//                       <ProductivityChart report={employeeReport} />
//                     </div>
  ...
  const loadData = async () => {
    try {
      if (isAdmin) {
        ...
        const todayStr = getLocalDateString(new Date());
        await Promise.all(filteredEmps.map(async (emp) => {
          try { reports[emp.id] = await api.getDailyReport(todayStr, emp.id); } catch { /* skip */ }
        }));
        setEmployeeReports(reports);
        ...
        setEmployeeReport(await api.getDailyReport());
      }
  ...
  useEffect(() => {
    const load = async () => {
      if (selectedEmployee && !showDetailModal) {
        try { setEmployeeReport(await api.getDailyReport(undefined, selectedEmployee.id)); } catch { setEmployeeReport(null); }
      }
    };
    load();
  }, [selectedEmployee]);
