...
function ReportsTab({ employees, employeeReports, fraudAlerts, dm }: { employees: UserInfo[]; employeeReports: Record<string, DailyReport>; fraudAlerts: FraudAlert[]; dm?: boolean }) {
  ...
  const handleExport = async (period: 'today' | 'weekly' | 'monthly') => {
    setReportLoading(period);
    try {
      if (period === 'today') {
        exportCSV(employees, employeeReports, fraudAlerts, 'daily');
      } else {
        const days = period === 'weekly' ? 7 : 30;
        const aggregatedReports: Record<string, DailyReport> = {};
        ...
        exportCSV(employees, aggregatedReports, fraudAlerts, period);
      }
    } catch (e) { console.error(e); }
  ...
  const reportCards = [
    { id: 'today' as const, title: 'Daily Report', desc: "Today's productivity data for all employees", icon: '📋', color: '#0F62FE' },
    { id: 'weekly' as const, title: 'Weekly Report', desc: 'Aggregated data for the past 7 days', icon: '📊', color: '#6929C4' },
    { id: 'monthly' as const, title: 'Monthly Report', desc: 'Aggregated data for the past 30 days', icon: '📈', color: '#198038' },
  ];
  ...
  return (
    <div className="p-8 pt-6 space-y-6 animate-fade-in">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {reportCards.map(rc => (
          <div key={rc.id} className={`border rounded-xl p-6 transition-all hover:shadow-lg group ${dm ? 'bg-gray-800/50 border-gray-700 hover:border-gray-600' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
            <div className="text-3xl mb-4">{rc.icon}</div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className={`text-xs font-semibold ${dm ? 'text-gray-500' : 'text-gray-400'}`}>{employees.length} employees</span>
              </div>
              <button
                onClick={() => handleExport(rc.id)}
                disabled={reportLoading !== null}
                className="flex items-center gap-2 px-4 py-2 text-white text-sm font-semibold rounded-lg transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                style={{ background: rc.color, boxShadow: `0 4px 15px ${rc.color}40` }}>
                {reportLoading === rc.id ? (
      ...
      <div className={`border rounded-xl p-6 ${dm ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
        <h3 className={`text-sm font-bold mb-4 flex items-center gap-2 ${dm ? 'text-white' : 'text-gray-900'}`}><Icons.FileText/> Report Preview — Today</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
